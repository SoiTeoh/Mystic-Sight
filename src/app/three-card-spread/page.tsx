...
export default function ThreeCardSpreadPage() {
  const [cards, setCards] = useState(Array(3).fill(null));
  ...
  const drawCard = useCallback((index: number) => {
    ...
    setCards(prev => {
      const newCards = [...prev];
      newCards[index] =  {...randomCard, isReversed: isReversed};
      return newCards;
    });
  ...
  const canDrawCard = (index: number) => cards[index] === null && drawnCardCount < 3;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-background text-foreground starry-background">
      ...
      <main className="flex-grow p-6 flex flex-col items-center justify-center gap-4">
        <div className="relative flex justify-center">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-16 md:gap-y-4">
                    ...
                    <div className="p-2 flex flex-col items-center">
                      <Image
                        ...
                        height={cardHeight - 180}
                         ...
                         className="md:max-w-none" //reset max-w on larger screens
                      />
        ...
        <div className="flex justify-center mt-12">
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" className="bg-gradient-to-r from-purple-500 to-purple-700 text-white rounded-full px-8 py-3 font-bold hover:shadow-lg transition-all" disabled={cards.filter(card => card !== null).length !== 3} onClick={handleReading}>
                {t('获取解读')}
              </Button>
            </DialogTrigger>
